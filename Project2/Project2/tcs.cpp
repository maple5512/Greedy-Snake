...
int main()
{
	...
	outtextxy(200, 40, _T("贪吃蛇"));
	outtextxy(220, 100, _T("简单"));
	outtextxy(220, 160, _T("一般"));
	outtextxy(220, 220, _T("困难"));
	rectangle(215, 95, 305, 145);
	rectangle(215, 155, 305, 205);
	rectangle(215, 215, 305, 265);
	MOUSEMSG m;
	while (state == 0)
	{
		m = GetMouseMsg();
		if (m.x > 215 && m.y > 95 && m.x < 305 && m.y < 145)
		{
			settextcolor(RED);
			outtextxy(220, 100, _T("简单"));
		...
		else if (m.x > 215 && m.y > 155 && m.x < 305 && m.y < 205)
		{
			settextcolor(RED);
			outtextxy(220, 160, _T("一般"));
			if (m.uMsg == WM_LBUTTONDOWN)
		...
		else if (m.x > 215 && m.y > 215 && m.x < 305 && m.y < 265)
		{
			settextcolor(RED);
			outtextxy(220, 220, _T("困难"));
			if (m.uMsg == WM_LBUTTONDOWN)
